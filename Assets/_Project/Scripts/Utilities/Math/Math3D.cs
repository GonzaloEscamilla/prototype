...
namespace _Project.Scripts.Utilities.Math
{
	public static class Math3d
	{
		...
		public static Vector3[] GetOrthonormalDistribution(Vector3 center, Vector3 forward, int amountOfPoints, float gap, Vector3 normal, float offset)
		{
			...
			Vector3 orthogonalVectorToLine = (Vector3.Cross(directrix.normalized, normal));

			Vector3[] points = new Vector3[amountOfPoints];

			for (int i = 0; i < amountOfPoints; i++)
			{
				points[i] = ((orthogonalVectorToLine * i) * gap + center) + (orthogonalVectorToLine.normalized * offset);
			}

			return points;
		}

		public static Vector3[] GetCircularDistribution(Vector3 center, int amountOfPoints, float radius)
		{
			Vector3[] distributedPositions = new Vector3[amountOfPoints];
			...
			for (int i = 0; i < amountOfPoints; i++)
			{
				...
				distributedPositions[i] = newPos;
			}

			return distributedPositions;
		}

		public static Vector3[] GetCircularDistribution(Vector3 center, int amountOfPoints, float radius, float clampAngle)
		{
			Vector3[] distributedPositions = new Vector3[amountOfPoints];
			clampAngle = clampAngle * Mathf.Deg2Rad;


			for (int i = 0; i < amountOfPoints; i++)
			{
				float angle = (i * ((Mathf.PI * 2f) - clampAngle)) / amountOfPoints;
				Vector3 newPos = new Vector3(Mathf.Cos(angle) * radius, 0, Mathf.Sin(angle) * radius) + center;
				distributedPositions[i] = newPos;
			}
		...
		public static Vector3[] GetCircularDistribution(Vector3 center, int amountOfPoints, float radius, float clampAngle, float offset)
		{
			Vector3[] distributedPositions = new Vector3[amountOfPoints];
			clampAngle = clampAngle * Mathf.Deg2Rad;
			...
			for (int i = 0; i < amountOfPoints; i++)
			{
				float angle = (i * ((Mathf.PI * 2f) - clampAngle) + offset) / amountOfPoints;
				Vector3 newPos = new Vector3(Mathf.Cos(angle) * radius, 0, Mathf.Sin(angle) * radius) + center;
				distributedPositions[i] = newPos;
			}
		...
		public static Vector2[] GetCircularDistribution(Vector2 center, int amountOfPoints, float radius, float clampAngle, float offset)
		{
			Vector2[] distributedPositions = new Vector2[amountOfPoints];
			clampAngle = clampAngle * Mathf.Deg2Rad;

			for (int i = 0; i < amountOfPoints; i++)
			{
				float angle = (i * ((Mathf.PI * 2f) - clampAngle) + offset) / amountOfPoints;
				Vector3 newPos = new Vector2(Mathf.Cos(angle) * radius, Mathf.Sin(angle) * radius) + center;
				distributedPositions[i] = newPos;
			}
		...
		public static Vector3[] GetCircularDistribution(Vector3 center, Vector3 forward, int amountOfPoints, float radius, float clampAngle, float offset)
		{
			if (clampAngle == 0)
				clampAngle = 1;
			...
			Vector3 direction = (forward - center).normalized;
			Vector3 rightFromCenter = (Vector3.right + new Vector3(0f, 0f, center.z)).normalized;

			float internalOffset = (Vector3.SignedAngle(rightFromCenter, direction,Vector3.up)) * Mathf.Deg2Rad;

			Vector3[] distributedPositions = new Vector3[amountOfPoints];

			for (int i = 0; i < amountOfPoints; i++)
			{
				float angle = ((i * ((Mathf.PI * 2f) - clampAngle) + offset) / amountOfPoints) - internalOffset;
				Vector3 newPos = new Vector3(Mathf.Cos(angle) * radius, 0, Mathf.Sin(angle) * radius) + center;
				distributedPositions[i] = newPos;
			}
		...
		public static Vector3[] GetCiruclarDirections(Transform center, int amountOfDirections, float planarRotationMultiplier)
		{
			Vector3[] directions = GetCircularDistribution(center.position, amountOfDirections, 1);

			for (int i = 0; i < directions.Length; i++)
			{
				directions[i] = (center.position - directions[i]) + (center.up * planarRotationMultiplier);
			}
			return directions;
		}

		public static Vector3[] GetCiruclarDirections(Vector3 center, int amountOfDirections, float planarRotationMultiplier)
		{
			Vector3[] directions = GetCircularDistribution(center, amountOfDirections, 1);

			for (int i = 0; i < directions.Length; i++)
			{
				directions[i] = (center - directions[i]) + ((Mathf.Abs(Vector3.Dot(center, Vector3.up)) * Vector3.up).normalized * planarRotationMultiplier);
			}
			return directions;
		}
